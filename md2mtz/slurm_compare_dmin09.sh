#!/bin/bash
#SBATCH --job-name=lunus_compare09
#SBATCH --partition=gpu
#SBATCH --nodelist=voltron
#SBATCH --gres=gpu:GV100:1
#SBATCH --ntasks=1
#SBATCH --time=60:00
#SBATCH --output=/home/jamesh/projects/lunus/lunus/md2mtz/slurm_compare_dmin09_%j.out
...
tmpdir=/dev/shm/${USER}/lunus_compare09_slurm_$$
mkdir -p $tmpdir
xtraj=${lunus}/lunus/lunus/command_line/xtraj.py

echo "=== GPU run (d_min=0.9, 3 frames) ==="
date
python $xtraj \
  top=${here}/amber_1000.pdb \
  traj=${here}/amber_1000.nc \
  first=0 last=2 d_min=0.9 \
  engine=gpu \
...
echo "=== CPU run (d_min=0.9, 3 frames) ==="
date
python $xtraj \
  top=${here}/amber_1000.pdb \
  traj=${here}/amber_1000.nc \
