...
def test_max_pairs_cpu_keeps_the_measured_default():
    pairs, why = recommended_max_pairs(CPU)
    assert pairs == MAX_PAIRS_CPU
    assert "measured" in why


def test_max_pairs_cuda_sizes_to_l2():
...
def test_max_pairs_unknown_l2_falls_back_rather_than_guessing():
...
def test_max_pairs_small_free_memory_caps_the_budget():
...
def test_max_pairs_never_below_the_measured_default():
    info = DeviceInfo(kind="cuda", l2_bytes=1 << 20, free_bytes=1 << 20)
    pairs, _ = recommended_max_pairs(info)
    assert pairs == MAX_PAIRS_CPU


def test_max_pairs_cap_cannot_raise_the_budget():
...
# ---- estimated_peak_bytes / memory_warning ----------------------------------

def test_peak_grows_as_dmin_cubed():
...
def test_peak_counts_pair_buffers_and_atoms():
...
def test_memory_warning_quiet_when_it_fits():
    assert memory_warning(10_000, (100, 100, 100), MAX_PAIRS_CPU, CUDA) is None


def test_memory_warning_fires_when_close():
...
def test_memory_warning_silent_without_a_free_memory_reading():
