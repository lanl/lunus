...
| `aniso.py` | Isotropic-background removal — the anisotropic component of a diffuse map. Shell-averages in reciprocal space, fits a not-a-knot cubic spline through the shell means and subtracts it. `subtract_isotropic()` is the one-shot (masked data supported); `spline_basis()` exposes the operation as the fixed linear map it is, for callers evaluating it repeatedly on one reflection list. No crystallography library needed — `xtraj.py`'s `to_aniso()` is now a cctbx-shaped wrapper around this. `AnisoOperator` is the whole of `to_aniso()` precomputed and batched — background subtraction plus an average over a fixed partition of the reflections (`segment_average()`, which reproduces a cctbx Laue merge and map-back bit for bit once the grouping is supplied). Built for leave-one-out sweeps, where `to_aniso()` would otherwise re-solve the spline and re-derive the merge mapping thousands of times. |
...
| `tune.py` | Sizes `max_pairs_per_batch` from the grid, the atom count and what the device reports about itself -- to the CUDA L2 rather than the CPU cache the 4M default was fitted to, capped against free memory -- and warns when the estimated peak approaches it. Every rule traces to a measurement in `docs/performance.md` or to a hardware query; where a number would have to be guessed it keeps the measured default and says so. Also holds `recommended_compile()`, the `torch.compile` break-even arithmetic, **deliberately unwired**: its inputs are one measurement on one machine and a per-frame saving that scales with resolution, so the threshold does not transfer. No torch import at module scope. |
...
| `test_aniso.py` | isotropic-background removal against a verbatim copy of the loop that used to live in `xtraj.to_aniso()`, asserted as exact equality — plus the torch operator's agreement with it, its linearity, and its gradients. |
| `test_aniso_operator.py` | `AnisoOperator` against `to_aniso()`: the segment average exactly, against a copy of cctbx's `merge_equivalents_real` summation order; the whole operator to ~1e-9, which is what the basis form costs. Includes the batched forms, the fixed mask, the torch `SegmentAverage`, and — where cctbx is present — parity with the live `to_aniso()` and `symmetry_average()`. |
| `test_xtraj_optimizer.py` | the frame-selection optimizer end to end: the whole greedy selection run through `sweep_candidates()`/`correlator()` and through a naive reference sharing none of their shortcuts, asserting the same frames in the same order — which covers the expanded diffuse expression, the blocking, complex64 storage, the reduced operator and the segment-weighted correlation at once. cctbx-gated. |
| `test_xtraj_smoke.py` | does xtraj still run — one two-frame calculation per engine (`cctbx`, `gemmi`, `torch`; `sfall` needs CCP4) on the tracked `examples/xtraj/` data, driven through the real command line, plus one `do_opt` run using the calculation's own diffuse map as the data so the answer is knowable. Smoke only: the cross-engine threshold is a correlation of 0.95, loose by design. Needs cctbx, mdtraj, h5py and gemmi. |
