...
| `tune.py` | Sizes `max_pairs_per_batch` from the grid, the atom count and what the device reports about itself -- to the CUDA L2 rather than the CPU cache the 4M default was fitted to, capped against free memory -- and warns when the estimated peak approaches it. Every rule traces to a measurement in `docs/performance.md` or to a hardware query; where a number would have to be guessed it keeps the measured default and says so. `torch_compile` is deliberately left alone: its break-even is a frame count, and the one measured on one machine did not transfer (2.8 frames on GB10 against ~210 on a faster card). No torch import at module scope. |
