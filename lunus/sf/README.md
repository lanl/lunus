...
| `tune.py` | Picks the torch engine's performance knobs from things known before the frame loop: frame count, grid shape, atom count, and what the device reports about itself. Decides `torch.compile` against its measured break-even (~210 frames on CUDA, ~2 on CPU, never on MPS) and sizes `max_pairs_per_batch` to the CUDA L2 rather than the CPU cache the 4M default was fitted to, capped against free memory. Every rule traces to a measurement in `docs/performance.md` or to a hardware query; where a number would have to be guessed it keeps the measured default and says so. No torch import at module scope. |
