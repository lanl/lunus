...
**There is one xtraj.** The torch-engine development variant that lived in `lunus/sf/xtraj.py` replaced the shipped dispatcher, so `lunus/command_line/xtraj.py` now carries every engine. Beyond the dispatcher's `cctbx`/`sfall`/`gemmi` it has `engine=torch`, plus `torch_device=`, `torch_num_threads=`, `torch_taper_width=`, `save_density=` and `use_top_bfacs=`. Three options default to `auto`, are resolved once the grid shape and the device are both known, and each prints the reason it chose what it did: `torch_blur=` (the FFT-artifact blur, sized from the grid and the model's minimum B — it matters a lot at low B, see `docs/solvent-design.md`), `torch_compile=` (against its measured break-even — ~210 frames *per process* on CUDA, ~2 on CPU, never on MPS) and `torch_max_pairs_per_batch=` (sized to the CUDA device's own L2 rather than the CPU cache the 4M default was fitted to, capped against free memory). The latter two live in `lunus/sf/tune.py`; an explicit value always wins and is reported as such.
