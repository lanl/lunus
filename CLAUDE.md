...
`lunus/command_line/xtraj.py` reads an MD trajectory (mdtraj) plus topology and computes ensemble structure-factor statistics: `<F>` (Bragg) and `<|F|²> − |<F>|²` (diffuse). Arguments are `key=value` on the command line, parsed positionally out of `sys.argv`. Key options: `top=`, `traj=`, `d_min=`, `first=`, `last=`, `chunk=`, `engine=` (`cctbx` default | `sfall` | `gemmi`), `gemmi_cutoff=` (the atomic density truncation, e/Å³ — badly named, since it is handed to the torch kernel as well as to gemmi's calculator; **default 1e-4** since engine parity, direct summation and the solvent R-factor all converged on it, and `fit_solvent_rfactor.py` calls the same quantity `--density-cutoff`), `cctbx_method=` (`fft` | `direct`). Output filenames are set with `fcalc=`, `icalc=`, `diffuse=`. `examples/xtraj/run.sh` is a working smoke test. Note that its reference outputs (`*.ref.mtz`, `*.ref.hkl`) are **not** in git — they are caught by the blanket `examples/xtraj/*.mtz` / `*.hkl` patterns in `.gitignore`, so a fresh clone has only `run.sh`, `top_ref.pdb` and `traj_ref.xtc`.

**There is one xtraj.** The torch-engine development variant that lived in `lunus/sf/xtraj.py` replaced the shipped dispatcher, so `lunus/command_line/xtraj.py` now carries every engine. Beyond the dispatcher's `cctbx`/`sfall`/`gemmi` it has `engine=torch`, plus `torch_device=`, `torch_compile=`, `torch_num_threads=`, `torch_taper_width=`, `save_density=` and `use_top_bfacs=`. Two options default to `auto`, are resolved once the grid shape and the device are both known, and print the reason they chose what they did: `torch_blur=` (the FFT-artifact blur, sized from the grid and the model's minimum B — it matters a lot at low B, see `docs/solvent-design.md`) and `torch_max_pairs_per_batch=` (sized to the CUDA device's own L2 rather than the CPU cache the 4M default was fitted to, capped against free memory — `lunus/sf/tune.py`). An explicit value always wins and is reported as such. `torch_compile=` is **not** auto-selected and defaults to True: its break-even is a frame count whose inputs don't transfer across machines (measured at ~210 frames per rank on one card and 2.8 on another), so short runs should set it False by hand.
